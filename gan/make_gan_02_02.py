...
# batch_size = 200 # 원본
batch_size = 9600
learning_rate = 0.0002    # 원본
# learning_rate = 0.7
# learning_rate = 0.002 # 에러
# 신경망 레이어 구성 옵션
n_hidden = 1000
n_input = 128*173
n_noise = 1000  # 생성기의 입력값으로 사용할 노이즈의 크기
...
def generator(noise_z):
    hidden = tf.nn.elu(tf.matmul(noise_z, G_W1) + G_b1)
    output = tf.nn.sigmoid(tf.matmul(hidden, G_W2) + G_b2)
    # output = tf.nn.softmax(tf.matmul(hidden, G_W2) + G_b2)
    return output
